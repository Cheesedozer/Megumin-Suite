...
4. [Short suggestion]
    ...
    {
      // No static ruleset here on purpose. This block's mechanics (the RNG queue,
      // the RollTheDice tool call, the turn-by-turn state memo, XP/level-up
      // directives) are produced by the Fatbody extension's own runtime — its
      // chat interceptor, its registered function-tool, its LLM-driven state
      // pass — not by prompt text. A frozen copy of that text can't reproduce
      // any of it standalone, so there's no meaningful default to fall back to.
      // When Fatbody is installed and enabled, index.js's
      // resolveFatbodyBlockContent() pulls its current rules live; otherwise
      // this block contributes nothing — see buildBaseDict().
      id: "fatbody",
      label: "Fatbody D&D",
      trigger: "[[FATBODY]]",
      content: ""
